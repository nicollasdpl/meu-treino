...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  carregarConfigsFirebase();
  carregarConfigsGitHub();
});
...
const btnSaveFbCfg = document.getElementById('btnSaveFbCfg');
const btnInitFirebase = document.getElementById('btnInitFirebase');
...
function carregarConfigsFirebase(){
  const cfg = localStorage.getItem('fbConfig');
  if(cfg){
    firebaseConfigText.value = cfg;
    habilitarInit(true);
  }
}
function habilitarInit(ok){
  if(btnInitFirebase) btnInitFirebase.disabled = !ok;
}
if(btnSaveFbCfg){
  btnSaveFbCfg.addEventListener('click', ()=>{
    try{
      const cfgStr = firebaseConfigText.value.trim();
      if(!cfgStr) return alert('Cole o JSON do firebaseConfig.');
      JSON.parse(cfgStr); // valida
...
if(btnInitFirebase){
  btnInitFirebase.addEventListener('click', ()=>{
    try{
      const cfgStr = localStorage.getItem('fbConfig');
      if(!cfgStr) return alert('Salve o firebaseConfig primeiro.');
      const firebaseConfig = JSON.parse(cfgStr);
      if(!fbApp){
        fbApp = firebase.initializeApp(firebaseConfig);
        auth = firebase.auth();
        db = firebase.firestore();
        userStatus.textContent = 'Firebase inicializado';
        btnLoginGoogle.disabled = false;
        auth.onAuthStateChanged(async (u)=>{
          fbUser = u || null;
          if(fbUser){
            userStatus.textContent = `Conectado: ${fbUser.displayName||fbUser.email}`;
            btnLogout.disabled = false;
            btnLoginGoogle.disabled = true;
            updateSyncInfo('Conectado');
            await cloudPullMerge(); // baixa e mescla
          }else{
    ...
    }catch(e){ alert('Erro ao iniciar Firebase: ' + e.message); }
  });
}
if(btnLoginGoogle){
...
if(btnLogout){
  btnLogout.addEventListener('click', async ()=>{ if(auth) await auth.signOut(); });
}

/* Firestore helpers */
function sessionsColl(){
  if(!db || !fbUser) return null;
  return db.collection('users').doc(fbUser.uid).collection('sessoes');
}
async function cloudUpsertSession(sess){
  // tenta Firebase, senão GitHub, senão nada
  if(db && fbUser){
  ...
  if(ghIsConfigured()){
    await ghUpsertAll(getSessoes()); // escreve snapshot atual
    return;
  }
  throw new Error('Sem provedor de nuvem configurado');
}
...
async function cloudPullMerge(){
  try{
    ...
    [...local, ...remoto].forEach(s=>{
      ...
      if(!ex) byDate.set(k, s);
      else byDate.set(k, (s.duracao||0) > (ex.duracao||0) ? s : ex);
    });
    const merged = [...byDate.values()].sort((a,b)=>a.data.localeCompare(b.data));
    setSessoes(merged);
    montarCalendario();
    updateSyncInfo('Sincronizado');
  }catch(e){ updateSyncInfo('Falha ao sincronizar'); }
}
...
const btnSyncGh = document.getElementById('btnSyncGh');

function carregarConfigsGitHub(){
  const cfg = JSON.parse(localStorage.getItem('ghCfg')||'{}');
  if(cfg.owner) ghOwnerEl.value = cfg.owner;
  if(cfg.repo)  ghRepoEl.value  = cfg.repo;
  if(cfg.path)  ghPathEl.value  = cfg.path;
  if(cfg.token) ghTokenEl.value = cfg.token;
  atualizarGhStatus();
}
function atualizarGhStatus(){
  const ok = ghIsConfigured();
  ghStatus.textContent = ok ? 'GitHub: pronto para sincronizar' : 'GitHub: não configurado';
}
function ghIsConfigured(){
  return ghOwnerEl.value && ghRepoEl.value && ghPathEl.value && ghTokenEl.value;
}
if(btnSaveGh){
  btnSaveGh.addEventListener('click', async ()=>{
    localStorage.setItem('ghCfg', JSON.stringify({
      ...
      path:  ghPathEl.value.trim(),
      token: ghTokenEl.value
    }));
    atualizarGhStatus();
    try{
      await ghUpsertAll(getSessoes());
      ghStatus.textContent = 'GitHub: teste OK (arquivo atualizado)';
    }catch(e){
      ghStatus.textContent = 'GitHub: erro ao salvar ('+e.message+')';
    }
  });
}
if(btnSyncGh){
  btnSyncGh.addEventListener('click', async ()=>{
    try{
      // baixa, mescla, salva
      const remoto = await ghFetchAll();
      const local  = getSessoes();
      const byDate = new Map();
      [...local, ...(Array.isArray(remoto)?remoto:[])].forEach(s=>{
        const k=s.data, ex=byDate.get(k);
        if(!ex) byDate.set(k,s); else byDate.set(k,(s.duracao||0)>(ex.duracao||0)?s:ex);
      });
      const merged = [...byDate.values()].sort((a,b)=>a.data.localeCompare(b.data));
      setSessoes(merged);
      await ghUpsertAll(merged);
      montarCalendario();
      ghStatus.textContent = 'GitHub: sincronizado';
    }catch(e){ ghStatus.textContent = 'GitHub: erro ('+e.message+')'; }
  });
}

async function ghGetFile(){
  const {owner, repo, path, token} = JSON.parse(localStorage.getItem('ghCfg')||'{}');
  ...
  return {sha:j.sha, content: j.content ? atob(j.content) : null};
}
async function ghUpsertAll(arr){
  const {owner, repo, path, token} = JSON.parse(localStorage.getItem('ghCfg')||'{}');
  if(!ghIsConfigured()) throw new Error('GitHub não configurado');
  const {sha} = await ghGetFile().catch(()=>({sha:null}));
  const content = btoa(unescape(encodeURIComponent(JSON.stringify(arr,null,2))));
  const body = {
    message: 'Atualiza sessões via app',
    content,
    branch: 'main',
    ...(sha? {sha}: {})
  };
  ...
  fileImport.addEventListener('change', async ()=>{
    ...
    try{
      ...
      if(Array.isArray(json)){
        ...
        try{ await cloudUpsertSession({}); }catch{}
      }else{
